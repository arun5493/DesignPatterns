...
The only constraint that I found in the system is that, since there are large amount of requests generated by the users as well as the servers, the user often gets 'unable to connect'. That is, too many pull requests are generated and thus it has become hard to manage the load resulting in the error.
